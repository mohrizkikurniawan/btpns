    SUBROUTINE BTPNS.VVR.BIFAST.CHK.SUSP
*-----------------------------------------------------------------------------
* Developer Name     : Hamka Ardyansah
* Development Date   : 1 Agustus 2022
* Description        : for validation in process activate for checking the account is have proxy status suspend
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
   ...
   IF Y.PROXY.STATUS EQ "ACTV" AND Y.PROXY.REG.ID NE '' THEN
        GOSUB DEFAULT.VALUE
   END ELSE
        AF = BF.PM.ACCOUNT.NO
        ETEXT = "EB-ERROR.PROXY.SUSP"
