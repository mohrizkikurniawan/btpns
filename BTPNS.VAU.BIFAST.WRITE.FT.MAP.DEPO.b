 SUBROUTINE BTPNS.VAU.BIFAST.WRITE.FT.MAP.DEPO
*-----------------------------------------------------------------------------
* Developer Name     : Moh Rizki Kurniawan
* Development Date   : 20221211
* Description        : takeout charges non bifast from pool bifast
*-----------------------------------------------------------------------------
...
*----------------------------------------------------------------------
    ...
    Y.APP     = "FUNDS.TRANSFER":FM:"IDIH.WS.DATA.FT.MAP":FM:"IDIH.IN.FT.JOURNAL.PAR" :FM: "FT.COMMISSION.TYPE"
...
*flag charge deposito bifast (non materai)

    vLocChargeAmt = ''    
    FINDSTR 'TXBIFASTCHG' IN Y.COMMISSION.TYPE SETTING vPosF, vPosV, vPosS THEN
        Y.COMMISSION.TYPE   = Y.COMMISSION.TYPE<vPosF, vPosV, vPosS>
        vLocChargeAmt       = vCommissionAmt<vPosF, vPosV, vPosS>
    END

    FINDSTR 'TXBIFASTEMP' IN Y.COMMISSION.TYPE SETTING vPosF, vPosV, vPosS THEN
        Y.COMMISSION.TYPE   = Y.COMMISSION.TYPE<vPosF, vPosV, vPosS>
        vLocChargeAmt       = vCommissionAmt<vPosF, vPosV, vPosS>
    END

    IF vLocChargeAmt NE '' THEN
        FINDSTR 'IDR' IN vLocChargeAmt SETTING vPosF1, vPosV1, vPosS1 THEN
            vLocChargeAmt = vLocChargeAmt[4,999]
        END
    END
...
*------------------------------------------------------------------------------------
	IF Y.ACC.CR.SPLIT NE '' THEN
		...
		R.IDIH.QUEUE.CHARGES<QUEUE.CHG1.AMOUNT>      = vLocChargeAmt
...
*------------------------------------------------------------------------------------
    ...
    IF vCommissionCode EQ "DEBIT PLUS CHARGES" THEN
        cntCommissionType = DCOUNT(vCommissionType, @VM)
        IF cntCommissionType EQ 1 THEN
			CALL F.READ(FN.FT.COMMISSION.TYPE,vCommissionType,R.FTCOMM,F.FT.COMMISSION.TYPE,ERR2)
			flagMaterai  = R.FTCOMM<FT4.LOCAL.REF, posFlagMaterai>
        END
    END

	IF R.NEW(FT.COMMISSION.CODE) EQ "WAIVE" OR flagMaterai NE '' THEN
