*-----------------------------------------------------------------------------
    SUBROUTINE BTPNS.ENR.DF.MRK1(valOutput)
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
    ...
    IF NOT(vFolder) THEN
        ENQ.DATA = "FOLDER is mandatory selection criteria"
    END ELSE
*        IF GETENV('HOME', vHome) THEN vHome = vHome : "/APPSHARED.BP/"
*        vFolder = vHome : vFolder
        fnFolder        = "../UD/APPSHARED.BP/LOAN/STATEMENT/" : vFolder
        fvFolder        = ""
        ...
        OPEN fnFolder TO fvFolder THEN
            ...
            IF noList LT 1 THEN
                ENQ.DATA    = vFolder : " is empty"
            END ELSE
                GOSUB PutData
            END
        END ELSE
            ENQ.DATA    = "Directory/folder " : vFolder : " is not exist"
        END
    END
