*-----------------------------------------------------------------------------------
	SUBROUTINE BTPNS.EBR.CUS.COLL.ID(ENQ.DATA)
*-----------------------------------------------------------------------------------
* Author	: Kania Lydia
* Usage		: Routine for build selection collateral id by CIF
* Date		: 20220524
* Reference	: 
*-----------------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
	...
	BEGIN CASE
	CASE R.CUSTOMER NE ""
		SEL.CMD		= "SELECT ":FN.COLLATERAL: " WITH @ID LIKE ":Y.REF.ID:"..."
		SEL.LIST	= ''
	...
	CASE R.ACCOUNT NE ""
		...
		SEL.CMD		= "SELECT ":FN.COLLATERAL: " WITH @ID LIKE ":Y.CUS.ID:"..."
		SEL.LIST	= ''								
		CALL EB.READLIST(SEL.CMD, SEL.LIST, '', NO.REC, ERR.AA)
		IF SEL.LIST THEN
			GOSUB PROCESS
...
PROCESS:
*-----------------------------------------------------------------------------
	Y.COLLATERAL.ID = SEL.LIST
	CONVERT FM TO " " IN Y.COLLATERAL.ID
	IF Y.COLLATERAL.ID THEN
        ENQ.DATA<2,Y.AV> = ""
        ENQ.DATA<3,Y.AV> = ""
        ENQ.DATA<4,Y.AV> = ""
        ...
        ENQ.DATA<2,Y.AV> = "@ID"
        ENQ.DATA<3,Y.AV> = "EQ"
        ENQ.DATA<4,Y.AV> = Y.COLLATERAL.ID
    END
