*-----------------------------------------------------------------------------------
	SUBROUTINE BTPNS.EBR.CUS.IS.CONTRACT(ENQ.DATA)
*-----------------------------------------------------------------------------------
* Author	: Kania Lydia
* Usage		: Routine for build selection IS.CONTRACT id by CIF
* Date		: 20220524
* Reference	: 
*-----------------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
	...
	FN.IS.CONTRACT = "F.IS.CONTRACT"
    F.IS.CONTRACT  = ""
    CALL OPF(FN.IS.CONTRACT, F.IS.CONTRACT)
	...
	BEGIN CASE
	CASE R.CUSTOMER NE ""
		SEL.CMD		= "SELECT ":FN.IS.CONTRACT: " WITH CUSTOMER EQ ":Y.REF.ID
		SEL.LIST	= ''
	...
	CASE R.ACCOUNT NE ""
		...
		SEL.CMD		= "SELECT ":FN.IS.CONTRACT: " WITH CUSTOMER EQ ":Y.CUS.ID
		SEL.LIST	= ''								
		CALL EB.READLIST(SEL.CMD, SEL.LIST, '', NO.REC, ERR.AA)
		IF SEL.LIST THEN
			GOSUB PROCESS
...
PROCESS:
*-----------------------------------------------------------------------------
	Y.IS.CONTRACT.ID = SEL.LIST
	CONVERT FM TO " " IN Y.IS.CONTRACT.ID
	IF Y.IS.CONTRACT.ID THEN
        ENQ.DATA<2,Y.AV> = ""
        ENQ.DATA<3,Y.AV> = ""
        ENQ.DATA<4,Y.AV> = ""
        ...
        ENQ.DATA<2,Y.AV> = "@ID"
        ENQ.DATA<3,Y.AV> = "EQ"
        ENQ.DATA<4,Y.AV> = Y.IS.CONTRACT.ID
    END
