	SUBROUTINE BTPNS.VAU.BIFAST.DEAC.PROXY.ID
*-----------------------------------------------------------------------------
* Developer Name     : Budi Saptono
* Development Date   : 20220729
* Description        : Auth Routine to Proxy Alias BIFAST DEAC
*-----------------------------------------------------------------------------
...
*construct date
   ...
   Y.REG.ID          = R.NEW(BF.PM.REGISTER.ID)
...
* ------------
   ...
   IF Y.RESPONSE.CODE.RES EQ 'ACTC' THEN
	  ...
	  Y.PROXY.ID.OLD = R.ACC<AC.LOCAL.REF, Y.PROXY.ID.POS>
	  
	  R.ACC<AC.LOCAL.REF, Y.PROXY.TYPE.POS>   = Y.PROXY.TYPE.NFMT
	  R.ACC<AC.LOCAL.REF, Y.PROXY.ID.POS>     = Y.PROXY.ID
	  R.ACC<AC.LOCAL.REF, Y.PROXY.REG.ID.POS> = Y.REG.ID.RES 
	  R.ACC<AC.LOCAL.REF, Y.PROXY.STATUS.POS> = Y.PROXY.STATUS
	  WRITE R.ACC TO F.ACC, Y.ACCT.NO

	  CALL F.READ(FN.PROXY.CONC, Y.PROXY.ID.OLD, R.PC, F.PROXY.CONC, CONC.ERR)
          R.PC<BF.PC.PROXY.STATUS> = Y.PROXY.STATUS
	  WRITE R.PC TO F.PROXY.CONC, Y.PROXY.ID.OLD
 
	  CALL F.READ(FN.PROXY.CONC, Y.PROXY.ID, R.PC, F.PROXY.CONC, CONC.ERR)
	  R.PC<BF.PC.PROXY.STATUS> = Y.PROXY.STATUS
	  R.PC<BF.PC.PROXY.TYPE>   = Y.PROXY.TYPE.NFMT
