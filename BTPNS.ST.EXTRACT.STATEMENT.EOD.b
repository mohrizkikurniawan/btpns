*------------------------------------------------------------------------------
    SUBROUTINE BTPNS.ST.EXTRACT.STATEMENT.EOD
*------------------------------------------------------------------------------
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
    ...
    nameText        = fromDate:"_DETAIL":batchDetails<1, 1, 1>:".csv"

    vAccountCnt     = DCOUNT(batchDetails, @SM)
    FOR idx = 1 TO vAccountCnt
...
*-----------------------------------------------------------------------------
	...
	CALL EB.ACCT.ENTRY.LIST(vGl, TODAY, TODAY, stmtEntryList, openBal, vErr)
    saldo     = openBal

    CALL F.READ(fnAcctEntLworkDay, vGl, rvAcctEntLworkToday, fvAcctEntLworkDay, '')
    cntIdEntry  = DCOUNT(rvAcctEntLworkToday, @FM)

    FOR idxEntry = 1 TO cntIdEntry
        idStmtEntry = rvAcctEntLworkToday<idxEntry>
        CALL F.READ(fnStmtEntry, idStmtEntry, rvStmtEntry, fvStmtEntry, '')
        amountLcy   = rvStmtEntry<StmtEntry_AmountLcy>
        amountLcy   = amountLcy*(-1)
        saldo       = saldo + (amountLcy)
    NEXT idxEntry

    
    FOR idxEntryDetail = 1 TO cntIdEntry
        idStmtEntry     = rvAcctEntLworkToday<idxEntryDetail>
        CALL F.READ(fnStmtEntry, idStmtEntry, rvStmtEntry, fvStmtEntry, '')
