    SUBROUTINE BTPNS.VAU.BIFAST.SEND.SUSP
*-----------------------------------------------------------------------------
* Developer Name     : Hamka Ardyansah
* Development Date   : 1 Agustus 2022
* Description      : auth routine for send message suspend proxy
*-----------------------------------------------------------------------------
  ...
  END ELSE
     IF Y.RESPONSE.CODE.RES.1 EQ '' THEN
      ...
      END ELSE
         ...
         ETEXT = "Error RJCT ":"Reason Err : ":Y.REASON.CODE.RES.1:"-":Y.REASON.CODE.DESC
         CALL STORE.END.ERROR
...
*-----------------------------------------------------------------------------
   ...
   END ELSE
      IF Y.RESPONSE.CODE.RES EQ '' THEN
         ...
         END
      END ELSE
         E = "Response Code : ":Y.RESPONSE.CODE.RES
         CALL ERR
      END
   END
