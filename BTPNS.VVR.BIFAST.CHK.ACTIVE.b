    SUBROUTINE BTPNS.VVR.BIFAST.CHK.ACTIVE
*-----------------------------------------------------------------------------
* Developer Name     : Hamka Ardyansah
* Development Date   : 29 July 2022
* Description     : for validation in process suspend for checking the acocunt is have proxy status active
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
   ...
   IF Y.PROXY.STATUS NE "ACTV" AND Y.PROXY.REG.ID NE '' THEN
	GOSUB CHECK.ACCOUNT
        IF Y.FLAG.PROC NE '1' THEN
            GOSUB DEFAULT.VALUE
        END
   END ELSE
        AF = BF.PM.ACCOUNT.NO
        ETEXT = "EB-ERROR.PROXY"
